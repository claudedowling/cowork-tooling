...
- `config.sh` — your instance settings (git identity, signing item reference,
  repo owners, host path). Copy `config.example.sh` and fill it in.
...
2. Copy `config.example.sh` to `config.sh` in the working folder and fill in
   your git identity, signing item, and repo owners.
3. Add `.credentials/op_token` (mode 600) and `.op-env`.
4. Download the `gh` and `op` CLIs into `bin/` as `gh-real` and `op`.
5. Each session: approve the file-delete tool once, then
   `bash bin/session-init.sh`.

## Security model

- **No secret values, and no personal identifiers, are committed.** Instance
  settings live in a gitignored `config.sh`; secrets stay in 1Password and
  `.credentials/`. The scripts themselves are generic.
- A `gitleaks` GitHub Action runs on every push and pull request and is a
  **required status check**; `main` is protected and requires signed commits,
  so nothing lands without a clean scan.
