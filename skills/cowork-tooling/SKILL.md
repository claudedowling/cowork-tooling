---
name: cowork-tooling
description: Use when operating in the cowork-tooling setup: starting a Cowork session (connect the tooling folder, run session-init), cloning or updating GitHub repos with use-repo, or changing tooling through the branch/PR/gitleaks workflow. Covers the credential model, safety rules, and the read-only bin/ deploy.
---

# cowork-tooling

Operating guide for the Cowork session tooling. Scripts live in `bin/`, deployed read-only from this repo's `main`.
